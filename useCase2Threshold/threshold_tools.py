...
def get_lmoments(ams, distr='gev', multiple_points=True):
    ...
    lmoments = xr.apply_ufunc(lmom_distr.lmom_fit, ams, input_core_dims=[["time"]], 
                              exclude_dims=set(("time",)), output_core_dims=[[]], dask='allowed')
...
def get_aicc_stat(ams, multiple_points=True):
    ...
    def aicc_stat(ams):
        ...
        all_aicc_results = str(aicc_gev) + str(aicc_gum) + str(aicc_wei) + str(aicc_pe3) + str(aicc_gpa)
        all_aicc_results_string = str(all_aicc_results)

        lowest_aicc_value = min(aicc_gev[1], aicc_gum[1], aicc_wei[1], aicc_pe3[1], aicc_gpa[1])
    ...
    all_aicc_results, lowest_aicc_distr, lowest_aicc_value = xr.apply_ufunc(aicc_stat, ams, 
...
def get_ks_stat(ams, distr='gev', multiple_points=True):
    ...
    def ks_stat(ams):
        ...
        if distr == 'gev':
            try:
                ...
                ks = stats.kstest(ams, 'genextreme', 
        ...
        if distr == 'weibull':
            try:
                ...
                ks = stats.kstest(ams, 'weibull_min', 
                                  args=(lmoments['c'], lmoments['loc'], lmoments['scale']))
                d_statistic = ks[0]
                p_value = ks[1]
            except ValueError:
    ...
    d_statistic, p_value = xr.apply_ufunc(ks_stat, ams, input_core_dims=[["time"]], 
                              exclude_dims=set(("time",)), output_core_dims=[[],[]], dask='allowed')
...
def get_return_value(ams, return_period=10, distr='gev', multiple_points=True):
    ...
    return_value = xr.apply_ufunc(return_value, ams, input_core_dims=[["time"]], 
                          exclude_dims=set(("time",)), output_core_dims=[[]], dask='allowed')
...
def get_return_prob(ams, threshold, distr='gev', multiple_points=True):
    ...
    return_prob = xr.apply_ufunc(return_prob, ams, input_core_dims=[["time"]], 
                              exclude_dims=set(("time",)), output_core_dims=[[]], dask='allowed')
    ...
    new_ds['return_prob'].attrs['threshold'] = 'exceedance of {} value event'.format(str(threshold))
    new_ds.attrs = ams_attributes
    new_ds.attrs['distribution'] = '{}'.format(str(distr))
...
def get_return_period(ams, return_value, distr='gev', multiple_points=True):
    ...
    return_period = xr.apply_ufunc(return_period, ams, input_core_dims=[["time"]], 
                          exclude_dims=set(("time",)), output_core_dims=[[]], dask='allowed')
    ...
    new_ds['return_period'].attrs['return value'] = 'occurrence of a {} value event'.format(str(return_value))
    new_ds.attrs = ams_attributes
    new_ds.attrs['distribution'] = '{}'.format(str(distr))
